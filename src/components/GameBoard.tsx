      ...
      <div className="min-w-[900px]">
        ...
        <div className="flex flex-col gap-1">
          {[...guesses].reverse().map((g, i) => {
            ...
            // Show every skin of this monster: awakened + unawakened + collab twins.
            const forms: { key: string; name: string; imageUrl: string | null; element: Element }[] = [];
            const addSkins = (m: ReturnType<typeof catalog.getById>) => {
              if (!m) return;
              forms.push({ key: `${m.id}`, name: m.name, imageUrl: m.imageUrl, element: m.element });
              if (m.altName) {
                forms.push({ key: `${m.id}-alt`, name: m.altName, imageUrl: m.altImageUrl, element: m.element });
              }
            };
            if (monster) {
              addSkins(monster);
              for (const id of monster.twinIds) addSkins(catalog.getById(id));
            }
            ...
            return (
              ...
              >
                ...
                >
                  {(forms.length ? forms : [{ key: 'none', name: `#${g.monsterId}`, imageUrl: null }]).map(
                    (f) => (
                ...
                >
                  {(forms.length
                    ? forms
                    : [{ key: 'none', name: `#${g.monsterId}`, element: null as Element | null }]
                  ).map((f) => (
                    <span key={f.key} className="flex items-center gap-1.5">
                      ...
                      <span className="break-words leading-tight">{f.name}</span>
                    </span>
                  ))}
                </div>
